...
-- Método Profesor
SELECT DISTINCT i.idInstructor, i.nombre, i.apellido, ca.nombre AS carrera_nombre
	FROM instructor i
	JOIN cohorte c ON (i.instructor = c.idInstructor)
...
SELECT a.nombre, 
	...
	INNER JOIN carrera ca
	ON c.idCarrera = ca.idCarrera
    -- 		Todas las respuestas a las preguntas de arriba:
    -- WHERE ca.nombre LIKE '%Stack dev%'
    -- WHERE ca.nombre NOT LIKE '%Science'
